        ...
        getShopFileManager().getYamlConfigurationMap().forEach((s, config) -> {
            ...
            for (String key : config.getConfigurationSection("Menu Items").getKeys(false)) {
                categoryItems.add(new ShopItem(
                        new ItemBuilder(XMaterial.matchXMaterial(config.getString("Menu Items." + key + ".Display Item.Material")).get().parseMaterial()).name(config.getString("Menu Items." + key + ".Display Item.Name")).data(config.getInt("Menu Items." + key + ".Display Item.Data")).appendLore(config.getStringList("Menu Items." + key + ".Display Item.Lore")).build(),
                        config.getBoolean("Menu Items." + key + ".Settings.Give Display"), config.getInt("Menu Items." + key + ".Settings.Price"), config.getInt("Menu Items." + key + ".Settings.Slot"), config.getStringList("Menu Items." + key + ".Settings.Commands"))
                );
            }
            shopMap.put(s,
